...
export const SystemHealth: React.FC = () => {
  ...
  const getHealthColor = (val: number, type: 'latency' | 'error' | 'apdex') => {
    if (type === 'latency') return val < 200 ? 'text-emerald-400' : val < 500 ? 'text-yellow-400' : 'text-rose-400';
    if (type === 'error') return val < 1 ? 'text-emerald-400' : val < 5 ? 'text-yellow-400' : 'text-rose-400';
    if (type === 'apdex') return val > 0.95 ? 'text-emerald-400' : val > 0.85 ? 'text-yellow-400' : 'text-rose-400';
  ...
  return (
    <div className="h-full overflow-y-auto p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-ops-panel border border-ops-border p-3 rounded">
          <div className="text-[10px] text-slate-500 uppercase font-bold mb-1">P95 Latency</div>
          <div className={`text-xl font-mono ${getHealthColor(metrics.latency, 'latency')}`}>
            {metrics.latency.toFixed(0)}ms
          </div>
          <div className="w-full bg-slate-800 h-1 mt-2 rounded-full overflow-hidden">
             <div className="h-full bg-current opacity-70 transition-all duration-500" style={{ width: `${Math.min(100, metrics.latency / 5)}%` }}></div>
          </div>
        </div>
        <div className="bg-ops-panel border border-ops-border p-3 rounded">
          <div className="text-[10px] text-slate-500 uppercase font-bold mb-1">Error Rate</div>
          <div className={`text-xl font-mono ${getHealthColor(metrics.errorRate, 'error')}`}>
            {metrics.errorRate.toFixed(2)}%
          </div>
          <div className="w-full bg-slate-800 h-1 mt-2 rounded-full overflow-hidden">
             <div className="h-full bg-current opacity-70 transition-all duration-500" style={{ width: `${Math.min(100, metrics.errorRate * 10)}%` }}></div>
          </div>
        </div>
        <div className="bg-ops-panel border border-ops-border p-3 rounded">
        ...
        <div className="bg-ops-panel border border-ops-border p-3 rounded">
          <div className="text-[10px] text-slate-500 uppercase font-bold mb-1">Apdex Score</div>
      ...
      <div className="p-4 border border-dashed border-ops-border rounded-lg bg-ops-panel/30">
        <div className="flex items-center gap-3 mb-2">
            ...
            <div>
                <h4 className="text-sm font-bold text-slate-200">Sentry Monitoring Active</h4>
                <p className="text-xs text-slate-500">Exceptions are automatically captured and sent to your project.</p>
            </div>
        </div>
